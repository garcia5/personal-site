        ...
        <div className="max-w-prose mb-40 text-center">
            ...
            <section className="pb-4">
                ...
                {workExperience.map((job, index) => (
                    <div key={index} className="bg-ctp-surface0 p-4 rounded-lg shadow-md text-left my-4">
                        <div className="flex justify-between items-start">
                            <p className="text-xl font-bold text-ctp-mauve">{job.company}</p>
                            <p className="text-sm text-ctp-subtext0 text-right">{job.dates}</p>
                        </div>
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {funStuff.map((item, index) => (
                        <div key={index} className="group relative rounded-lg shadow-md overflow-hidden">
                            ...
                            <div className="absolute inset-0 bg-black opacity-60 flex items-center justify-center transition-opacity duration-300 group-hover:opacity-0">
                                <h3 className="font-bold text-2xl text-ctp-text">{item.title}</h3>
                            </div>
                        </div>
                    ))}
                </div>
